import { atom, nanoid, buildFlowchartCode, Node, Edge, FlowchartData } from './deps';
...
export const addNodeAtom = atom(
  null,
  (get, set, newNode: { type: Node['type']; position: { x: number; y: number }; label: string; parentId?: string }) => {
...
export const updateNodeAtom = atom(
  null,
  (get, set, update: { id: string; position?: { x: number; y: number }; data?: { label: string } }) => {
    const nodes = get(nodesAtom);
...
export const addEdgeAtom = atom(
  null,
  (get, set, newEdge: { source: string; target: string; type: Edge['type']; label?: string }) => {
    const edges = get(edgesAtom);
    const edge: Edge = {
      id: nanoid(),
...
export const removeEdgeAtom = atom(
  ...
  (get, set, edgeId: string) => {
    const edges = get(edgesAtom);
    set(edgesAtom, edges.filter(edge => edge.id !== edgeId));
  }
);